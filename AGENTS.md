...
- 管理端登录成功后应进入统一主页面，该主页面负责承载首页、当前已接入业务模块以及后续新增模块的统一布局与导航入口
...
- 优先保证模块边界清晰，避免过早抽象
- 仓库已有稳定模式时，优先复用既有约定
- 管理端新增业务模块时，应优先接入登录成功后的统一主页面，不得为单一模块重复建设独立首页框架
- 根据所修改的层级补充对应测试或验证步骤
- `exam-service` 的 controller 测试应聚焦 REST 契约、鉴权/鉴权失败、参数校验、HTTP 状态码、响应结构与 `TraceNo` 等接口边界；service 测试应聚焦业务规则、状态流转、持久化副作用、异常分支与关键幂等行为；mapper/custom SQL 测试应聚焦查询条件、聚合统计、软删除过滤与唯一性约束；config 测试应聚焦拦截器、异常映射、日志与通用基础设施行为
- `exam-service` 的测试不得长期只停留在 controller 层；凡是存在明确业务规则、状态流转、唯一性校验、导入导出处理、事务边界或定时任务行为的后端改动，必须补充对应的 service 层直接测试，不能只通过 controller 集成测试间接覆盖
