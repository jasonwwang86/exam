# score-generation Specification

## Purpose
定义 `exam-service` 在本次范围内支撑自动判分、成绩结果生成、结果快照幂等落库、补偿重试与脱敏日志的约束，确保既有 `exam-web` 考试列表与成绩详情能力能直接复用结果；本次不扩展到人工阅卷、成绩发布审批、动态大屏或新的考生端模块。
